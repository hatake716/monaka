...
| 横画面 | 縦画面と同じ配置 | **補助キー1段 + 薄型バー**でターミナル領域を最大化 |
...
- **補助キーを 1 段に**：縦画面では 7 キー × 2 段（固定幅・横スクロール）ですが、横画面は横幅に余裕があるので 14 キーすべてを 1 段に等分割して並べます。全キーが一望でき、横スクロールも要りません。
- **上部バーを薄型化**：`Button` 既定の最小高さ（48dp）と内部パディングを外し、32〜34dp の帯に収めています。
- 分割画面などで幅が足りずラベルが読めなくなる場合は、自動的に 2 段 + 横スクロールへ戻します。

Pixel 10a の横画面（923 × 411dp）では合わせて **92dp（画面高の約 22%）** をターミナルに回せます。フォント 15dp なら約 5 行ぶんです。
